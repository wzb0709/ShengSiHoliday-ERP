...
const NoticeModal: FC<IProps> = (props) => {
    ...
    >
      <FormItem label='公告标题' {...formItemLayout}>
        {getFieldDecorator('notice_title', {
          initialValue: props.initialValue.notice_title,
          rules: [
            {
              required: true,
              message: '请填写公告标题',
        ...
        })(<Input placeholder='请填写公告标题' style={{ width: '90%' }} />)}
      </FormItem>
      <FormItem label='公告正文' {...formItemLayout}>
        {getFieldDecorator('notice_content', {
          initialValue: props.initialValue.notice_content ? BraftEditor.createEditorState(props.initialValue.notice_content) : undefined,
          rules: [{
            required: true,
...
export default Form.create<IProps>()(NoticeModal)
