...
const Expense:FC = (props) => {
  ...
  const columns: ColumnProps<Object>[] = [
            ...
            : <Badge status='error' text='审核拒绝' /> },
    {
      dataIndex: '', title: '操作', render: recode => <Fragment>
        <Link to={`/money/myExpense/${recode.id}`}>查看详情</Link>
      </Fragment>,
    },
  ]
  ...
  const getExpenseList = useCallback(() => {
    expenseServices.getExpenseList(params.search,params.status,params.start_time,params.end_time,page,size)
      .then((res: any) => {
        setDataSource(res.data)
        setCount(res.count)
        expenseServices.getStatistical(1,params.search,params.status,params.start_time,params.end_time).then(res=>{
          setStatistical(res)
        })
      })
  }, [page, size, params])
